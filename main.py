...
def rezult_calc():
    res = None
    second_num = int(line_imput.get())
    line_imput.delete(0, END)
    if operator == "+":
    ...
    line_imput.insert(0, res)


def choise_number(num):
    line_imput.insert(END, num)


def choise_operator(oper):
    global operator, first_num
    first_num = int(line_imput.get())
    operator = oper
    line_imput.delete(0, END)
...
line_imput = Entry(window, font=("Arial, 14"))
line_imput.grid(row=0, column=0, columnspan=4, sticky="ew")
